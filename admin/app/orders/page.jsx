"use client";
import { useEffect, useState } from "react";
import Loading from "@/components/Loading";
import { orderDummyData } from "@/assets/assets";
import { useAdmin } from "@/context/AdminContext";

export default function StoreOrders() {
  const { orders, loading, error, fetchOrders,updateOrderStatus } = useAdmin();
  const [selectedOrder, setSelectedOrder] = useState(null);
  const [isModalOpen, setIsModalOpen] = useState(false);

  const handleUpdateOrderStatus = async (orderId, status) => {
    try {
      await updateOrderStatus(orderId, status);
      toast.success("Statut mis à jour avec succès");
      
      // Update selected order if modal is open
      if (selectedOrder && selectedOrder._id === orderId) {
        setSelectedOrder(prev => ({ ...prev, status }));
      }
    } catch (error) {
      toast.error(error.message || "Erreur lors de la mise à jour");
    }
  };

  const openModal = (order) => {
    setSelectedOrder(order);
    setIsModalOpen(true);
  };

  const closeModal = () => {
    setSelectedOrder(null);
    setIsModalOpen(false);
  };

  useEffect(() => {
    fetchOrders({ limit: 100 });
  }, [fetchOrders]);

  if (loading.orders) return <Loading />;

  if (error.orders) {
    return (
      <div className="p-4 text-red-500">
        Erreur lors du chargement des commandes: {error.orders}
      </div>
    );
  }

  return (
    <>
      <h1 className="text-2xl text-slate-500 mb-5">
        Liste des <span className="text-slate-800 font-medium">Commandes</span>
      </h1>
      {orders.length === 0 ? (
        <p>Aucune commande</p>
      ) : (
        <div className="overflow-x-auto max-w-4xl rounded-md shadow border border-gray-200">
          <table className="w-full text-sm text-left text-gray-600">
            <thead className="bg-gray-50 text-gray-700 text-xs uppercase tracking-wider">
              <tr>
                {["N°", "Client", "Total", "Paiement", "Status", "Date"].map(
                  (heading, i) => (
                    <th key={i} className="px-4 py-3">
                      {heading}
                    </th>
                  )
                )}
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {orders.map((order, index) => (
                <tr
                  key={order._id}
                  className="hover:bg-gray-50 transition-colors duration-150 cursor-pointer"
                  onClick={() => openModal(order)}
                >
                  <td className="pl-6 text-green-600">{index + 1}</td>
                  <td className="px-4 py-3">{order.user?.name}</td>
                  <td className="px-4 py-3 font-medium text-slate-800">
                    DT {order.totalAmount.toFixed(2)}
                  </td>
                  <td className="px-4 py-3">{order.deliveryType === 'delivery' ? 'Livraison' : 
                 order.deliveryType === 'pickup' ? 'Retrait' : 'En magasin'}</td>

                  <td
                    className="px-4 py-3"
                    onClick={(e) => {
                      e.stopPropagation();
                    }}
                  >
                    <select
                      value={order.status}
                      onChange={(e) =>
                        updateOrderStatus(order._id, e.target.value)
                      }
                      className="border-gray-300 rounded-md text-sm focus:ring focus:ring-blue-200"
                    >
                      <option value="pending">Pending</option>
                      <option value="processing">Processing</option>
                      <option value="shipped">Shipped</option>
                      <option value="delivered">Delivered</option>
                      <option value="cancelled">Cancelled</option>
                    </select>
                  </td>
                  <td className="px-4 py-3 text-gray-500">
                    {new Date(order.createdAt).toLocaleString()}
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      )}

      {/* Modal */}
      {isModalOpen && selectedOrder && (
        <div
          onClick={closeModal}
          className="fixed inset-0 flex items-center justify-center bg-black/50 text-slate-700 text-sm backdrop-blur-xs z-50"
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="bg-white rounded-lg shadow-lg max-w-2xl w-full p-6 relative"
          >
            <h2 className="text-xl font-semibold text-slate-900 mb-4 text-center">
              Détails
            </h2>

            {/* Customer Details */}
            <div className="mb-4">
            
              <p>
                <span className="text-green-700">Client:</span>{" "}
                {selectedOrder.user?.name}
              </p>
              <p>
                <span className="text-green-700">Email:</span>{" "}
                {selectedOrder.user?.email}
              </p>
              <p>
                <span className="text-green-700">Téléphone:</span>{" "}
                {selectedOrder?.phone}
              </p>
              <p>
                <span className="text-green-700">Addresse:</span>{" "}
                {`${selectedOrder.shippingAddress?.street}, ${selectedOrder.shippingAddress?.city}, ${selectedOrder.shippingAddress?.postalCode}`}
              </p>
            </div>

            {/* Products */}
            <div className="mb-4">
              <h3 className="font-semibold mb-2">Products</h3>
              <div className="space-y-2">
                {selectedOrder.items.map((item, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-4 border border-slate-100 shadow rounded p-2"
                  >
                    {item.product?.images?.[0]?.url && (
                      <img
                        src={item.product.images[0].url} // ✅ Utilise .url au lieu de .src
                        alt={item.product.images[0].alt || item.name}
                        className="w-16 h-16 object-cover rounded"
                      />
                    )}
                    <div className="flex-1">
                      <p className="text-slate-800">{item.product?.name}</p>
                      <p>Qté: {item.quantity}</p>
                      <p>Prix unitaire: {item.price.toFixed(2)} DT</p>
                      <p className="font-medium text-slate-700">
                        Sous-total: {item.subtotal.toFixed(2)} DT
                      </p>
                    </div>
                  </div>
                ))}
              </div>
            </div>

            {/* Payment & Status */}
             <div className="mb-4 space-y-1">
              <p>
                <span className="text-green-700">Numéro de commande:</span>{' '}
                {selectedOrder.orderNumber}
              </p>
              <p>
                <span className="text-green-700">Type de livraison:</span>{' '}
                {selectedOrder.deliveryType === 'delivery' ? 'Livraison' : 
                 selectedOrder.deliveryType === 'pickup' ? 'Retrait' : 'En magasin'}
              </p>
              <p>
                <span className="text-green-700">Méthode de paiement:</span>{' '}
                {selectedOrder.paymentMethod === 'cash_on_delivery' 
                  ? 'Paiement à la livraison' 
                  : selectedOrder.paymentMethod === 'bank_transfer'
                  ? 'Virement bancaire'
                  : selectedOrder.paymentMethod || 'N/A'}
              </p>
              <p>
                <span className="text-green-700">Frais de livraison:</span>{' '}
                {selectedOrder.shippingCost.toFixed(2)} DT
              </p>
              <p>
                <span className="text-green-700">Status:</span>{' '}
                <span className={`font-medium ${
                  selectedOrder.status === 'delivered' ? 'text-green-600' :
                  selectedOrder.status === 'cancelled' ? 'text-red-600' :
                  selectedOrder.status === 'shipped' ? 'text-blue-600' :
                  'text-yellow-600'
                }`}>
                  {selectedOrder.status}
                </span>
              </p>
              <p>
                <span className="text-green-700">Date de commande:</span>{' '}
                {new Date(selectedOrder.orderDate).toLocaleString('fr-FR')}
              </p>
              {selectedOrder.notes && (
                <p>
                  <span className="text-green-700">Notes:</span>{' '}
                  {selectedOrder.notes}
                </p>
              )}
              <div className="pt-2 mt-2 border-t border-slate-200">
                <p className="font-bold text-lg">
                  <span className="text-green-700">Total:</span>{' '}
                  {selectedOrder.totalAmount.toFixed(2)} DT
                </p>
              </div>
            </div>

            {/* Actions */}
            <div className="flex justify-end">
              <button
                onClick={closeModal}
                className="px-4 py-2 bg-slate-200 rounded hover:bg-slate-300"
              >
                Close
              </button>
            </div>
          </div>
        </div>
      )}
    </>
  );
}
